...
class ThreadYield extends Thread{
	...
	public boolean work = true; //반복문을 제어할 목적으로 만든 변수
	
	public ThreadYield(String name) {
		//쓰레드에는 기본적으로 name속성이 있고, Thread 생성자 중에서는 name값을 매개변수로 받아
		// 설정하는 생성자가 있다
		super(name); // 쓰레드의 이름 설정 / 이름을 부여하지 않으면 디폴트값으로 설정 -> thread_1, thread_2 이런식
	}
	@Override
	public void run() {
		while(!stop) { // stop 변수값이 true이면 반복 정지
			if(work) {
				//작업하는 영역
